...
def _build_datasets(manifest_path: Path, dataset_root: Path, img_size: int, batch_size: int, seed: int):
    ...
    def decode_with_fallback(path, label):
        def _load_py(path_tensor):
            # tf.EagerTensor/bytes/str -> Python-String
            if isinstance(path_tensor, (bytes, bytearray)):
                p = path_tensor.decode('utf-8')
            elif isinstance(path_tensor, str):
                p = path_tensor
            else:
                p = path_tensor.numpy().decode('utf-8')

            try:
                # Erst TF-Decoder versuchen (JPEG/PNG/GIF/BMP)
                img_bytes = tf.io.read_file(p).numpy()
                img = tf.image.decode_image(img_bytes, channels=3, expand_animations=False).numpy()
                ok = 1
            except Exception:
                # Fallback: Pillow -> nach JPEG re-encoden
                try:
                    with Image.open(p).convert('RGB') as im:
                        buf = io.BytesIO()
                        im.save(buf, format='JPEG')
                        img = tf.image.decode_jpeg(buf.getvalue(), channels=3).numpy()
                        ok = 1
                except Exception as e:
                    print(f"[skip] {p}: {e}")
                    img = np.zeros((1, 1, 3), dtype=np.uint8)  # Dummy
                    ok = 0
            return img, np.int32(ok)

        img, ok = tf.py_function(func=_load_py, inp=[path], Tout=[tf.uint8, tf.int32])
        img.set_shape([None, None, 3])
        ok.set_shape(())

        # deine Normalisierung wie gehabt:
        if IMAGE_NORMALIZER == 'resize':
            img = tf.image.resize(img, (img_size, img_size), method=tf.image.ResizeMethod.BILINEAR)
        elif IMAGE_NORMALIZER == 'crop':
            h = tf.shape(img)[0]; w = tf.shape(img)[1]
    ...
    def make_ds(paths, labels, training: bool):
        ...
        ds = ds.map(decode_with_fallback, num_parallel_calls=AUTOTUNE)
        ds = ds.filter(lambda img, label, ok: tf.equal(ok, 1))
        ds = ds.map(lambda img, label, ok: (img, label), num_parallel_calls=AUTOTUNE)
        if training:
