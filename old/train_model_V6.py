...
def _preprocess_tf_more_regularization(path, label_id, training=False, img_size=IMG_SIZE, normalizer=IMAGE_NORMALIZER):
    """Balanced augmentation for better convergence.

    - Geometric: left-right flip, light translate/zoom (small pad + random crop)
    - Photometric: mild brightness/contrast/saturation jitter (no hue by default)
    - Random erasing: small hole with low probability
    """
    img_bytes = tf.io.read_file(path)
    img = tf.io.decode_image(img_bytes, channels=3, expand_animations=False)
    ...
    if training:
        ...
        # 1) Geometric: flip + light translate/zoom (pad + random crop)
        x = tf.image.random_flip_left_right(x)
        pad = 8  # ~3.5% margin
        x = tf.image.resize_with_pad(x, img_size + pad, img_size + pad)
        x = tf.image.random_crop(x, size=[img_size, img_size, 3])

        # 2) Mild photometric jitter
        x = tf.image.random_brightness(x, max_delta=0.06)
        x = tf.image.random_contrast(x, lower=0.93, upper=1.07)
        x = tf.image.random_saturation(x, lower=0.93, upper=1.07)

        # 3) Random erasing / cutout (p=0.25), small patch
        def apply_cutout(y):
            frac = tf.random.uniform([], 0.05, 0.12)
            erase_h = tf.cast(tf.round(frac * tf.cast(img_size, tf.float32)), tf.int32)
            erase_w = tf.cast(tf.round(frac * tf.cast(img_size, tf.float32)), tf.int32)
            y1 = tf.random.uniform([], 0, img_size - erase_h + 1, dtype=tf.int32)
            x1 = tf.random.uniform([], 0, img_size - erase_w + 1, dtype=tf.int32)

            h_idx = tf.range(img_size)
            w_idx = tf.range(img_size)
            mask_h = tf.logical_or(h_idx < y1, h_idx >= y1 + erase_h)
            mask_w = tf.logical_or(w_idx < x1, w_idx >= x1 + erase_w)
            mask2d = tf.cast(mask_h[:, None], tf.float32) * tf.cast(mask_w[None, :], tf.float32)
            mask3d = mask2d[..., None]
            fill_val = tf.random.uniform([], 0.0, 1.0)
            return y * mask3d + fill_val * (1.0 - mask3d)

        x = tf.cond(tf.random.uniform([]) < 0.25, lambda: apply_cutout(x), lambda: x)
...
def _make_dataset(pairs, training=False):
    ...
    # No aditional regularization
    ds = ds.map(lambda p, y: _preprocess_tf(p, y, training), num_parallel_calls=num_calls)
    """
    # with aditional regularization
    if training:
        ds = ds.map(lambda p, y: _preprocess_tf_more_regularization(p, y, training), num_parallel_calls=num_calls)
    else:
        ds = ds.map(lambda p, y: _preprocess_tf(p, y, training), num_parallel_calls=num_calls)
    """
...
def main():
    ...
    V3
    # L2-regularized head (commented block above is the old head)
    inputs = tf.keras.Input(shape=(IMG_SIZE, IMG_SIZE, 3), name="input_1")
    x = base_model(inputs, training=False)
    x = layers.GlobalAveragePooling2D()(x)
    x = layers.Dropout(DROPOUT)(x)
    ...
    # Log metrics to training log in the requested order
    log_data_training(f"last-layer bias: {np.round(b, 4)}")
    log_data_training(f"mean max prob: {float(np.mean(maxp))}")
    log_data_training(f"share predicted class==1: {float((np.array(y_pred)==1).mean())}")
